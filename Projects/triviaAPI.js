    ...
    {   
       ...
       let correctGuessesThisTurn = 0;
        ...
        if (correctGuess)
        {
            ...
            prizeMoneyPrior = prizeMoney;
            prizeMoney += (currentLetterValue * correctGuessesThisTurn);
            $("body").append(`<h1>${prizeMoneyPrior} + ${prizeMoney - prizeMoneyPrior} is ${prizeMoney}</h1>`);
            //if they got it right, the box changed colors and nothing else to do.  Will change it to adding dollar amounts and stuff later in coding.      
        }
        else
        {
            prizeMoneyPrior = prizeMoney;
            prizeMoney += (currentLetterValue * correctGuessesThisTurn);
            $("body").append(`<h1>${prizeMoneyPrior} + ${prizeMoney - prizeMoneyPrior} is ${prizeMoney}</h1>`);
            //if they got it wrong, alert that they got it wrong.  Will change to next players turn later in coding.
            $currentBox.css("opacity",0.5);
